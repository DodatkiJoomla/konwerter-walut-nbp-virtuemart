...
class convertNBP_Kursy_Sprzedazy
{
    public $archive = true;
    public $last_updated = '';
    ...
    public function init()
    {
        ...
        $currencies = $cache->call(array('convertNBP_Kursy_Sprzedazy', 'getWaluty'), $this->ostatnie_kursy);
        ...
        if ($contents != false) {
            ...
            foreach ($lista_walut as $wartosc) {
                if (count($wartosc) == 5) {
                    ...
                    $kurs = (string)((float)$wartosc->kurs_sprzedazy / (float)$wartosc->przelicznik);
                    $waluty[$kod] = $kurs;
                    unset($wartosc);
                }
            }
